...
cols_label(
  label = "Variable", # want on the left side
  p.value = "P-Value",
  stat_1 = "Control"
) %>%  
...
table_w_plots <- lobsters %>%
  ...
  summarize(
      total_cpue = sum(cpue, na.rm = TRUE),
      #want to plot distrubution as a list
...
library(cowsay)
library(beepr)
say("All done making some beautiful tables! :) ", "cat"); beep(3)  
